...
{
	FMCPZipArchiveReader& FileReader = *InOutResult.Reader;
...
FImportInspectionResult::FImportInspectionResult(TUniquePtr<FMCPZipArchiveReader> InReader)
...
TOptional<FImportInspectionResult> InspectFileForImport(const FString& InPathToZip)
{
	...
	FImportInspectionResult Result(MakeUnique<FMCPZipArchiveReader>(ZipFile, InPathToZip));
...
static bool ImportFromZip(
	const FMCPZipArchiveReader& InReader, const TArray<FString>& InFilesInZipArchive, const FString& InSandboxTargetRoot, 
	...
	)
{
	...
	for (int32 Index = 0; Index < InFilesInZipArchive.Num() && bFreeOfErrors; ++Index)
	{
		...
		FileUtilities::FMCPZipFileMetaData Metadata;
