...
namespace PersistDetail
{
//-----5.8에서 추가된 "AttemptDeleteAndCreate" enum 분기 대응 함수
static bool CopyFileWithDeleteFallback(
		IPlatformFile& InPlatformFile,
		const TCHAR* InToFilename,
		const TCHAR* InFromFilename)
{
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
	return InPlatformFile.CopyFile(
		InToFilename,
		InFromFilename,
	...
	// UE 5.8 EPlatformFileWrite::AttemptDeleteAndCreate compatibility:
	// if writing the destination fails, try deleting it and copying again.
	if (InPlatformFile.FileExists(InToFilename))
	{
		if (InPlatformFile.DeleteFile(InToFilename))
		{
			return InPlatformFile.CopyFile(
				InToFilename,
				InFromFilename,
...
//-----5.8에서 추가된 "AttemptDeleteAndCreate" enum 분기 대응 함수 (END)
...
{
	if (!CopyFileWithDeleteFallback(InPlatformFile,InToFilename, InFromFilename))
	{
