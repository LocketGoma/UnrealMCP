...
FTransform UEditorAppToolset::GetCameraTransform()
{
	...
	{
		UE::MCP::Compatibility::RaiseScriptError(TEXT("There is no current level camera."));
...
void UEditorAppToolset::FocusOnActors(const TArray<AActor*>& Actors)
{
	...
	{
		UE::MCP::Compatibility::RaiseScriptError(TEXT("FocusOnActors is not supported during PIE."));
		return;
	}
...
FVector UEditorAppToolset::ScreenCoordsToWorld(FVector2D Coords, float TraceDistance)
{
	...
	{
		UE::MCP::Compatibility::RaiseScriptError(FString::Printf(
	...
	if (!bHit)
	{
		UE::MCP::Compatibility::RaiseScriptError(TEXT("Unable to find a solid surface at the given screen coordinates."));
		return FVector::ZeroVector;
	}
...
void UEditorAppToolset::OpenEditorForAsset(const FString& AssetPath)
{
	...
	if (!Asset)
	{
		UE::MCP::Compatibility::RaiseScriptError(
...
FToolsetImage UEditorAppToolset::CaptureEditorImage()
{
	...
	if (Captured.IsEmpty())
	{
		UE::MCP::Compatibility::RaiseScriptError(TEXT("Failed to capture any editor windows."));
	...
	{
		UE::MCP::Compatibility::RaiseScriptError(TEXT("Failed to encode editor screenshot."));
		return FToolsetImage{};
	}
	return Image;
}
...
FMatrix UEditorAppToolset::GetWorldToClipMatrix(const FViewMatrices& ViewMatrices)
{
 ...
 * engine-accurate world-to-clip matrix from FSceneView::ViewMatrices so projected points
 ...
 */
TOptional<FMatrix> UEditorAppToolset::GetAnnotationViewProjectionMatrix(FEditorViewportClient* ViewportClient)
{
	...
	return GetWorldToClipMatrix(SceneView->ViewMatrices);
}
	...
	bool bShowUI)
{
	...
	{
		UE::MCP::Compatibility::RaiseScriptError(TEXT("Viewport has zero size."));
		return Out;
	}
	...
	{
		UE::MCP::Compatibility::RaiseScriptError(TEXT("Failed to capture viewport framebuffer."));
		return Out;
	}
	...
	if (!Annotations.IsSet())
	{
		...
		{
			UE::MCP::Compatibility::RaiseScriptError(TEXT("Failed to encode viewport screenshot."));
		}
		return Out;
	}
	...
	{
		UE::MCP::Compatibility::RaiseScriptError(TEXT("Annotated viewport capture requires a renderable scene; none is available."));
		return Out;
	}
	...
	if (!Out.Image.SetFromBitmap(Bitmap, Size))
	{
		UE::MCP::Compatibility::RaiseScriptError(TEXT("Failed to encode annotated viewport screenshot."));
		return Out;
	}
