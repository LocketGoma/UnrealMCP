...
private:
	 ...
	 * Uses GetWorldToClip() on UE 5.8 and later, or the equivalent
	 * GetViewProjectionMatrix() on earlier versions, including UE 5.7.4.
	 * @param ViewMatrices The scene view matrices to read from.
	 * @return The matrix that transforms world-space positions into clip space.
	 */
	static FMatrix GetWorldToClipMatrix(const FViewMatrices& ViewMatrices);
	static TOptional<FMatrix> GetAnnotationViewProjectionMatrix(FEditorViewportClient* ViewportClient);
